package com.example.hibernatedemo.entity;

import org.hibernate.annotations.NaturalId;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.OneToOne;
import java.math.BigDecimal;

@Entity
public class Vas2 {
    @Id
    @GeneratedValue(strategy= GenerationType.AUTO)
    private Integer id;

    @NaturalId
    @Column(name = "cn_book_id", nullable = false)
    private Long cnBookId;

    private BigDecimal chargePerUnit;

    @NaturalId
    private Boolean isActive;

    @OneToOne
    @JoinColumn(name = "cn_book_id", insertable = false, updatable = false)
    private ConsignmentGraph consignmentGraph;

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public Long getCnBookId() {
        return cnBookId;
    }

    public void setCnBookId(Long cnBookId) {
        this.cnBookId = cnBookId;
    }

    public BigDecimal getChargePerUnit() {
        return chargePerUnit;
    }

    public void setChargePerUnit(BigDecimal chargePerUnit) {
        this.chargePerUnit = chargePerUnit;
    }

    public Boolean getActive() {
        return isActive;
    }

    public void setActive(Boolean active) {
        isActive = active;
    }
}
